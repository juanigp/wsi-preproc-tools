...
import torch
import openslide
from utils import get_hue_based_mask
...
def get_args():
    ...
    parser.add_argument("--downsamples_dir", type=str, default='./', help='Directory with the downsampled .jpgs')
...
def save_wsi_foreground_patches(slide_fn, mask_fn, downsample_fn, tile_size, zoom_level, output_folder):
    slide = openslide.OpenSlide(slide_fn)
    mask = np.load(mask_fn)
    # downsample = Image.open(downsample_fn)
    downsample = cv2.imread(downsample_fn)
    downsample = cv2.cvtColor(downsample, cv2.COLOR_BGR2RGB)
    ...
    bool_foreground_mask = mask > 0
    bool_hue_based_mask = get_hue_based_mask(downsample, bool_foreground_mask) > 0
    ...
    valid_pixels = downsample[bool_foreground_mask]
    Io, alpha, beta =240, 1, 0.15
    HE, _, maxC = normalizer.compute_matrices(valid_pixels, Io, alpha, beta)
    ...
    for index, is_foreground in np.ndenumerate(foreground_patches_idcs):
        if is_foreground:
            ...
            # print(patch.shape)
            patch = normalizer.normalize(patch, HE, maxC, Io, alpha, beta, False)[0]
...
def pipeline_single_image(fn, args):
    ...
    # downsample_fn = None if args.downsamples_dir == '' else args.downsamples_dir
    downsample_fn = os.path.join(args.downsamples_dir, fn.replace(args.extension, '.jpg'))
    save_wsi_foreground_patches(slide_fn, mask_fn, downsample_fn, args.patch_size, args.zoom_level, slide_output_dir)
    return fn
...
def main(args):
    ...
    files = [f for f in os.listdir(args.slides_dir) if args.extension in f]
